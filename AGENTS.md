# Directives & Contraintes d'Environnement — realestatestbarthelemy.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **realestatestbarthelemy.com** (realestatestbarthelemy) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `realestatestbarthelemy.com` |
| **URL Cloudflare Pages** | `https://realestatestbarthelemy.pages.dev` |
| **Dépôt GitHub** | `jc842/realestatestbarthelemy` (branche principale : `main`) |
...
| **Matomo Site ID** | `64` |
| **Matomo MTM Container** | `container_jb4xU5OC.js` (Container ID : `jb4xU5OC`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `117` |
| **Zones Publicitaires** | Header (`702`), In-Content (`703`), Mobile Sticky (`704`), Large Rectangle (`701`), Half Page (`700`), Skyscraper (`705`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_jb4xU5OC.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 703).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 701).
